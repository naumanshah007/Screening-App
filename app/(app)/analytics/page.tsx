...
import {
  getClinicalAuthorityMonitoringSummary,
  type ClinicalAuthorityMonitoringSummary,
} from "@/lib/clinical-rules/monitoring";
...
function ClinicalAuthorityMonitoringPanel({
  summary,
}: {
  summary: ClinicalAuthorityMonitoringSummary;
}) {
  const metrics = [
    ...
    ["Recommendation reversals", summary.counts.recommendationReversals, "Append-only audit history"],
  ...
  const hardTriggerCount =
    ...
    summary.counts.adapterFailures +
    summary.counts.authorityResolutionFailures +
    summary.counts.urgentDisagreements +
    summary.counts.deEscalationBlocks +
    summary.counts.databaseFailures;

  return (
    <Card>
      <CardHeader>
        <div>
          <CardTitle>Clinical Authority Monitoring</CardTitle>
          <p className="mt-1 text-sm text-muted-foreground">
            Real events recorded since {summary.since.toLocaleDateString("en-NZ")} · no synthetic history or fabricated trend line.
          </p>
        </div>
        <Badge variant={hardTriggerCount > 0 ? "urgent" : "info"}>
          {hardTriggerCount > 0 ? `${hardTriggerCount} proposed hard trigger(s)` : "No proposed hard trigger observed"}
        </Badge>
      </CardHeader>
      <CardContent className="space-y-5">
          ...
          {metrics.map(([label, value, detail]) => (
            <div key={label} className="rounded-xl border border-border bg-muted/25 p-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={cn("text-xl font-bold", value > 0 && /failure|disagreement|anomal|DB|de-escalation/i.test(label) ? "text-destructive" : "text-foreground")}>{value}</span>
              </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-border">
          <table className="w-full min-w-[42rem] text-left text-xs">
            <thead className="bg-muted/50 text-muted-foreground">
              <tr><th className="px-3 py-2.5">Rollback signal</th><th className="px-3 py-2.5">Candidate threshold</th><th className="px-3 py-2.5">Approval state</th></tr>
            </thead>
            <tbody className="divide-y divide-border">
              {summary.thresholds.map((threshold) => (
                <tr key={threshold.signal}>
                  <td className="px-3 py-2.5 font-medium text-foreground">{threshold.signal}</td>
                  <td className="px-3 py-2.5 text-muted-foreground">{threshold.proposed}</td>
                  <td className="px-3 py-2.5"><Badge variant="high">PROPOSED · UNSIGNED · RISK OWNER APPROVAL REQUIRED</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <p className="text-xs leading-5 text-muted-foreground">
          Measurement is automatic. Rollback remains a manual, governed action; this panel does not autonomously change clinical authority.
        </p>
      </CardContent>
    </Card>
  );
...
export default async function AnalyticsPage() {
  ...
  const [
    ...
    getClinicalAuthorityMonitoringSummary(30),
  ]);
  ...
  return (
    <PageShell width="wide">
      ...
      <ClinicalAuthorityMonitoringPanel summary={authorityMonitoring} />
