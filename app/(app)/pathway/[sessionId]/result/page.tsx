...
const riskConfig: Record<string, {
  ...
  URGENT: {
    bg: "bg-destructive/5", border: "border-destructive/30", borderLeft: "border-l-destructive",
    title: "text-foreground",
    icon: <AlertTriangle className="h-6 w-6 text-destructive" />,
  },
  HIGH: {
    bg: "bg-warn/5", border: "border-warn/30", borderLeft: "border-l-warn",
    title: "text-foreground",
    icon: <AlertTriangle className="h-6 w-6 text-warn" />,
  },
  MEDIUM: {
    bg: "bg-info/5", border: "border-info/30", borderLeft: "border-l-info",
    title: "text-foreground",
    icon: <ClipboardList className="h-6 w-6 text-info" />,
  },
  LOW: {
    bg: "bg-success/5", border: "border-success/30", borderLeft: "border-l-success",
    title: "text-foreground",
    ...
    <button
      ...
      className={cn(
        ...
        sent
          ...
          : "border-border bg-card text-muted-foreground hover:border-brand-500 hover:text-brand-700 hover:shadow-sm",
        ...
        <div className="page-aura max-w-3xl mx-auto space-y-5">
          ...
          <div className={cn(
            ...
            <div className="flex items-start gap-4">
              <div className="p-2.5 rounded-xl bg-card/60 flex-shrink-0 shadow-sm">
                {cfg.icon}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  <RiskBadge risk={risk} size="md" />
                  <span className="text-sm font-medium text-muted-foreground">{figureLabel}</span>
                </div>
                <h1 className={cn("text-xl font-bold leading-snug mb-3", cfg.title)}>
                  {formatClinicalReferenceText(decision.recommendation)}
                </h1>
                {decision.recommendationCode && (
                  <span className="inline-block text-[11px] font-mono text-muted-foreground bg-card/60 px-2 py-0.5 rounded-md">
                    {decision.recommendationCode}
                  </span>
                )}
            ...
            <Card>
              <CardContent className="py-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Next Action</p>
                    <p className="text-sm font-semibold text-foreground">{formatClinicalReferenceText(decision.nextAction)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                  ...
                  <div className="space-y-2">
                    ...
                    {decision.referralType && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-foreground">{decision.referralType}</span>
                      </div>
                    )}
                    {decision.referralReason && (
                      <p className="text-xs text-muted-foreground mt-2 pt-2 border-t border-border leading-relaxed">
                        {formatClinicalReferenceText(decision.referralReason)}
                      </p>
                    )}
                  </div>
                </CardContent>
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <button
              ...
              className="w-full flex items-center gap-2 px-5 py-4 hover:bg-muted/40 transition-colors text-left"
              aria-expanded={guidelineOpen}
            >
              <BookOpen className="h-4 w-4 text-muted-foreground" />
              <span className="text-sm font-medium text-muted-foreground flex-1">Guideline reference &amp; clinical rationale</span>
              <ChevronRight className={cn("h-4 w-4 text-muted-foreground transition-transform duration-200", guidelineOpen && "rotate-90")} />
            </button>
            {guidelineOpen && (
              <div className="px-5 pb-5 space-y-4 border-t border-border pt-4 animate-slide-down">
                {decision.recommendationCode && (
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Recommendation Code</p>
                    <p className="font-mono text-sm text-muted-foreground">{decision.recommendationCode}</p>
                  </div>
                )}
                <div>
                ...
                {decision.rationale && (
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">{formatClinicalReferenceText(decision.rationale)}</p>
                  </div>
                )}
                {decision.guidelineReference && (
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Guideline</p>
                    <p className="text-sm text-muted-foreground">{formatClinicalReferenceText(decision.guidelineReference)}</p>
                  </div>
                )}
              </div>
            )}
