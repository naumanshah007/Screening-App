...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Retirement reason is required" }, { status: 400 });
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to retire version" }, { status: 409 });
  }
}
