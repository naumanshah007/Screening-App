...
[
  ...
  ["F5-CONFIRMED-ASC-H-TREATMENT-RECOMMENDED", ["ASC-H confirmed", "treatment recommended"], "Proceed with specialist-recommended treatment.", "TREATMENT", null],
  ...
  ["F5-TREATMENT-DEFERRED-HPV-NOT-DETECTED-TOC", ["treatment deferred", "HPV not detected", "no visible lesion"], "Begin/continue Test of Cure co-testing.", "TEST_OF_CURE", "co-testing sequence"],
...
] .forEach(([ruleId, conditions, expectedAction, actionClass, interval]) => add({ ruleId: ruleId as string, figureOrTable: "Figure 5", page: 47, recommendationNumbers: [], entryCriteria: ["HPV detected", "cytology ≥ASC-H", "normal colposcopy"], requiredInputs: ["MDM outcome", "treatment decision", "HPV/cytology/visible-lesion follow-up facts"], branchConditions: conditions as string[], expectedAction: expectedAction as string, actionClass: actionClass as string, referralRequired: actionClass === "TREATMENT", referralDestination: actionClass === "TREATMENT" ? "colposcopy/specialist treatment service" : null, guidelineTimeframe: interval as string | null, repeatInterval: interval as string | null, mandatoryClinicianReview: true, clinicianOnly: true, missingDataBehaviour: "CLINICIAN_REVIEW_REQUIRED; do not infer MDM, lesion, biopsy, or treatment decisions.", rationale: "Figure 5 depends on specialist review and cannot be autonomously finalised." }));
...
] .forEach(([ruleId, conditions, expectedAction, actionClass, referralDestination, interval]) => add({ ruleId: ruleId as string, figureOrTable: "Figure 10", page: 83, recommendationNumbers: [], entryCriteria: ["abnormal inter-menstrual or post-coital bleeding"], exclusionCriteria: ["do not substitute routine screening for symptom investigation"], requiredInputs: ["bleeding type/history", "speculum and pelvic examination", "cervix appearance", "cancer suspicion", "co-test", "OCP/STI assessment", "resolution at review"], branchConditions: conditions as string[], expectedAction: expectedAction as string, actionClass: actionClass as string, referralRequired: Boolean(referralDestination), referralDestination: referralDestination as string | null, guidelineTimeframe: interval as string | null, repeatInterval: interval && (interval as string).includes("6–8") ? "6–8 weeks" : null, mandatoryClinicianReview: ["URGENT_GYNAECOLOGY", "COLPOSCOPY", "LOCAL_PATHWAY_REVIEW", "GYNAECOLOGY"].includes(actionClass as string), clinicianOnly: ["URGENT_GYNAECOLOGY", "LOCAL_PATHWAY_REVIEW", "GYNAECOLOGY"].includes(actionClass as string), missingDataBehaviour: "Do not invent examination or work-up facts. Cancer concern overrides pending co-test; other missing work-up facts remain incomplete.", rationale: "Figure 10 is a symptom-investigation pathway and takes precedence over routine screening." }));
