...
/**
 * Organisation-scoped connector configuration for Phase 3A.
 *
 * The deployed path creates one new table and indexes. It never rebuilds an
 * existing table and it deliberately leaves IntegrationValidation untouched.
 */
async function ensureIntegrationConnectionSchema(
  client: ReturnType<typeof createClient>
) {
  await client.execute(`
    CREATE TABLE IF NOT EXISTS "IntegrationConnection" (
      ...
      "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      "updatedAt" DATETIME NOT NULL,
      CONSTRAINT "IntegrationConnection_organisationId_fkey"
        FOREIGN KEY ("organisationId") REFERENCES "Organisation"("id")
        ON DELETE RESTRICT ON UPDATE CASCADE,
      CONSTRAINT "IntegrationConnection_createdByUserId_fkey"
        FOREIGN KEY ("createdByUserId") REFERENCES "User"("id")
        ON DELETE RESTRICT ON UPDATE CASCADE,
      CONSTRAINT "IntegrationConnection_updatedByUserId_fkey"
        FOREIGN KEY ("updatedByUserId") REFERENCES "User"("id")
        ON DELETE RESTRICT ON UPDATE CASCADE
    )
  `);
  await client.execute(
    'CREATE UNIQUE INDEX IF NOT EXISTS "IntegrationConnection_organisationId_name_key" ON "IntegrationConnection"("organisationId", "name")'
  );
  await client.execute(
    'CREATE INDEX IF NOT EXISTS "IntegrationConnection_organisationId_state_updatedAt_idx" ON "IntegrationConnection"("organisationId", "state", "updatedAt")'
  );
  await client.execute(
    'CREATE INDEX IF NOT EXISTS "IntegrationConnection_organisationId_connectorType_idx" ON "IntegrationConnection"("organisationId", "connectorType")'
  );
  await client.execute(
